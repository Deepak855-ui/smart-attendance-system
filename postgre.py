...
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(minutes=20)  # Set session lifetime to 20 minutes
...
db = pymysql.connect(
                     host="mysql-222021-pablomaxico262-e9e3.k.aivencloud.com", 
                     user="avnadmin", 
                     password="AVNS_43Lml6tI2qJOkrKmX0d", 
...
def check_email():
    ...
    cursor = db.cursor()
    cursor.execute("""
        SELECT email FROM login_students WHERE email = %s 
        UNION 
